...
def split_prefix(word)
	...
	prefix = possible_prefixes.select{|prefix| $prefix_frequencies_hash[prefix].to_f >= 0.90 }.max_by{|prefix| $prefix_frequencies_hash[prefix].to_f } || ""
...
def split_suffix(word)
	...
	suffix = possible_suffixes.select{|suffix| $suffix_frequencies_hash[suffix].to_f >= 0.90 }.max_by{|suffix|  $suffix_frequencies_hash[suffix].to_f } || ""
...
def syllabify(word)
	...
	combinations.each do |combination|
		...
		score = get_precomputed_score( hyphenated_word , combinated_word , combination) ||	0
		score = 0 if has_illegal_syllable?(combinated_word)
...
def get_score( hyphenated_word , permutation , combination)	
	parts = hyphenated_word.split("-")
	probabilities = parts.map( )
	score = probabilities.inject(:*)
	score
end


def has_illegal_syllable?(permutation)
	x = permutation.split("-").select{|syllable| $syllable_existence_frequencies_hash[syllable].to_i == 0 }.size > 0
end
